...
var Queue = module.exports = function(name, connection) {
    this.name = name;
    ...
    var keepAlive = function() {
        ...
        kid = setTimeout(function() {
            ...
            var error = {
                ...
                error: {
                    status: 505,
                    message: "keep alive, didn't got any response from worker"
                },
                id: data.id
            };
            deferred.reject(error);
...
Queue.prototype.process = function(opts, handler) {
    ...
    var subscribe = function() {
        if (subscribed) {
            console.log("namespace '" + self.name + "' subscriber already waiting, return");
            return;
        }
        subscribed = true;

        console.log("subscribing namespace '" + self.name + "': total[" + self.totalWorkers + "], running[" + self.runningWorkers + "]");
        ...
        var sid = self.connection.subscribe(self.name, {queue:self.name}, function(toBeConvertedPayload, replyTo) {
            ...
            d.run(function() {
                ...
                console.log("namespace '" + self.name + "' got job: total[" + self.totalWorkers + "], running["  + self.runningWorkers + "]");
                ...
                handler(job, function(err, response) {
                    ...
                    console.log("namespace '" + self.name + "' job finished: total[" + self.totalWorkers + "], running[" + + self.runningWorkers + "]");
                    subscribe();
