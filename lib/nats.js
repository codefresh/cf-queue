...
Client.prototype.parseOptions = function(opts) {
  ...
  if (Array.isArray(options.url)) {
    options.url.forEach(function(server) {
        servers.push(url.parse(server));
    });
  } else {
    servers.push(url.parse(options.url));
  }
...
Client.prototype.createConnection = function() {
  ...
  console.log('Connecting to : ' + activeServerUrl.hostname + ":" + activeServerUrl.port);
  client.pongs   = [];
  client.pending = [];
  client.pSize   = 0;
  ...
  stream.once('connect', function() {
    console.log('Connected to : ' + activeServerUrl.hostname + ":" + activeServerUrl.port);
  ...
  stream.on('close', function(hadError) {
    console.log('Closed : ' + activeServerUrl.hostname + ":" + activeServerUrl.port);
    client.closeStream();
  ...
  stream.on('error', function(exception) {
    console.log('Error : ' + activeServerUrl.hostname + ":" + activeServerUrl.port);
    client.closeStream();
    ...
    while (!client.closed && client.inbound && client.inbound.length > 0) {
      ...
      case AWAITING_CONTROL:
        if ((m = MSG.exec(client.inbound)) !== null) {
          ...
          console.error(">>>> Got error message from NATS server: " + client.inbound);
